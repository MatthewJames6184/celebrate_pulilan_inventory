    ...
    return (
        <PublicTopicPage headTitle={headTitle} title={title} imageSubHeader={imageSubHeader} breadcrumbs={breadcrumbs}>
            <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <section className="mt-8 grid gap-6 xl:grid-cols-[290px_minmax(0,1fr)]">
                    <aside className="space-y-6">
                        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                            <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.24em] text-slate-700">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                {alphabet.map((alphabetLetter) => (
                                    <button
                                        key={alphabetLetter}
                                        type="button"
                                        onClick={() => setLetter(alphabetLetter === letter ? 'all' : alphabetLetter)}
                                        className={cn(
                                            'min-w-9 rounded-full border px-3 py-2 text-sm font-semibold transition',
                                        ...
                                        aria-pressed={letter === alphabetLetter}
                                    >
                                        {alphabetLetter}
                                    </button>
                                ))}
                    ...
                    <div className="space-y-5">
                        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-600">
                                {/* <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-800">Name browse: {letter === 'all' ? 'All letters' : letter}</span> */}
                                <span className="rounded-full bg-slate-100 px-3 py-1">Search: {query.trim() || 'Any keyword'}</span>
                            </div>
                        </div>
